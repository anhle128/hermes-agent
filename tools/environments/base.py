...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        # parent shell PID in ``&``-launched subshells, and macOS' system bash
        # 3.2 does not define ``$BASHPID``. Use mktemp with a quoted template so
        # every writer gets an actual distinct path on both Darwin and Linux.
        # ``command`` bypasses any shell function/alias named mktemp.
        ...
        bootstrap = (
            ...
            f" > \"$__hermes_snap_tmp\" && mv -f \"$__hermes_snap_tmp\" {_quoted_snap} "
            "|| { rm -f \"$__hermes_snap_tmp\"; exit 1; }\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # truncated/half-written file. ``$$`` is not unique across backgrounded
        # subshells and macOS' system bash 3.2 lacks ``$BASHPID``, so use mktemp
        # instead of shell PID expansion. Static path shlex-quoted for
        # Windows/spaces. ``command`` bypasses any shell function/alias named mktemp.
        _snap_tmp_template = shlex.quote(self._snapshot_path + ".tmp.XXXXXX")
